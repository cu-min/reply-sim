...
|--------|------|
| `login` | 微信静默登录 + 新用户创建 |
| `getScenarios` | 获取剧本列表（支持分类筛选，分页全量拉取，不受单次100条限制） |
...
| 家庭冲突 | 我其实一直在后悔 | "听说你现在想重新考研？" |
| 家庭冲突 | 你根本不懂我想什么 | "听说你不想继续考研了？" |
...
| Phase 6 | 新增家庭冲突剧本 × 2 + 聊天页顶栏返回按钮 + 候选回复卡片重设计 + getScenarios 分页 | 🚧 进行中 |
